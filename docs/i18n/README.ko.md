...
**Claude Code**의 MCP 서버 설정과 지시 파일(CLAUDE.md)을 **Gemini CLI**, **Codex CLI**, **OpenCode**, **Kiro CLI**, **Cursor**, **Kimi CLI**, **Vibe CLI**(Mistral), **Qwen Code**, **Amp**(Sourcegraph), **Cline CLI**, **Windsurf**, **Aider CLI**로 동기화합니다.
...
Claude Code를 주요 AI 코딩 에이전트로 사용하면서 무료 티어나 다른 모델을 활용하기 위해 다른 에이전트(Gemini CLI, Codex CLI, OpenCode, Kiro, Cursor, Kimi CLI, Vibe CLI, Qwen Code, Amp, Cline CLI, Windsurf, Aider CLI)도 함께 사용한다면, 각 도구마다 MCP 설정 형식이 다르고 하나씩 설정하는 것이 얼마나 번거로운지 아실 겁니다.
...
# 지시 파일 동기화 (CLAUDE.md → GEMINI.md / AGENTS.md / Kiro steering / Cursor rules / Amp AGENTS.md / Qwen AGENTS.md / Aider conventions)
...
| **Qwen Writer** | JSON → JSON, `type: "http"` → `httpUrl`, `${VAR}` → `$VAR` (Gemini와 동일한 패턴) |
| **Amp Writer** | JSON → JSON, `"amp.mcpServers"` 점 키 사용, `${VAR}` 유지 |
| **Cline Writer** | Claude와 동일한 형식 (Claude format) |
...
| Qwen | `~/.qwen/AGENTS.md` | `./AGENTS.md` (Codex / OpenCode / Kimi / Vibe와 공유) | 그대로 복사 (독립 라인 `@import` 확장) |
| Amp | `~/.config/amp/AGENTS.md` | `./AGENTS.md` (Codex / OpenCode / Kimi / Vibe / Qwen와 공유) | 그대로 복사 (독립 라인 `@import` 확장) |
...
| Qwen Code (글로벌) | `~/.qwen/settings.json` | JSON |
| Amp (글로벌) | `~/.config/amp/settings.json` | JSON |
| Cline CLI (글로벌) | `~/.cline/data/settings/cline_mcp_settings.json` | JSON |
| Windsurf (글로벌) | `~/.codeium/windsurf/mcp_config.json` | JSON |
...
| Cline CLI | 지시 동기화 미지원 | 지시 동기화 미지원 | — |
| Windsurf | 지시 동기화 미지원 | 지시 동기화 미지원 | — |
